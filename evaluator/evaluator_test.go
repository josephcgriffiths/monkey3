...
func TestArrayLiterals(t *testing.T) {
	...
	if len(result.Elements) != 3 {
		t.Fatalf("array has wrong number of elememts. got=%d", len(result.Elements))
	}
...
func TestHashLiterals(t *testing.T) {
	...
	for expectedKey, expeexpectedValue := range expected {
		pair, ok := result.Pairs[expectedKey]
		if !ok {
			t.Errorf("no pair for given key in Pairs.")
		}

		testIntegerObject(t, pair.Value, expeexpectedValue)
	}
}
